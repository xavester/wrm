...
function ds_custom_excerpt($temp_content='', $temp_max=800, $read_more = 1) { //Custom excerpt
	...
	switch ($read_more) :
		...
		case 1 :
			...
			$temp_content = apply_filters('the_content', $temp_content . '... ' . '<a href="' . get_permalink($post->ID) .  '" >Read More</a>');			
...
function exclude_child_cats($exclude_this_cat) {
	...
	foreach ($exclude_cats as $excluded_cats) :
		$result .= $excluded_cats->cat_ID . ',';
	endforeach;
	return $result;
